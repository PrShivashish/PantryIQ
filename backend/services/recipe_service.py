...
try:
    ...
    from accurate_image_service import get_accurate_image_service
...
class RecipeService:
    ...
    def __init__(self):
        ...
        # Initialize accurate image service for recipe-specific images
    ...
    async def search_recipes_with_algorithms(
        ...
        # Use simple service that works (OPTIMIZED)
        ...
        # Enhance recipes with graph theory for substitutions and better scoring if ingredients are provided
        if available_ingredients:
            recipes = self._apply_graph_theory_analysis(recipes, available_ingredients)
        
        # Enhance all recipes with accurate images
        recipes = self._enhance_recipes_with_accurate_images(recipes)
    ...
    def _apply_graph_theory_analysis(self, recipes: List[Dict], available_ingredients: List[str]) -> List[Dict]:
        ...
        for recipe in recipes:
            ...
            # Update recipe with graph analysis results (preserve existing accurate matching fields)
            # Only update if not already set by accurate matching algorithm
            if "match_score" not in recipe or recipe.get("algorithm_used") != "themealdb_accurate_matching":
                recipe["match_score"] = graph_match_score
                recipe["algorithm_used"] = "graph_theory"
            
            # Only update missing_ingredients if not set by accurate matcher
            ...
            # Add substitution suggestions (these are additive)
            if substitution_suggestions:
                existing_subs = recipe.get("substitution_suggestions", {})
                existing_subs.update(substitution_suggestions)
                recipe["substitution_suggestions"] = existing_subs
    ...
    def _enhance_recipes_with_accurate_images(self, recipes: List[Dict]) -> List[Dict]:
        """
        Enhance recipes with accurate, recipe-specific images (OPTIMIZED)
        """
        logger.info("🖼️ Enhancing recipes with accurate images...")
        ...
        for recipe in recipes:
            ...
            if cache_key in self.image_cache:
                recipe['image_url'] = self.image_cache[cache_key]
                ...
                # Cache the image
                self.image_cache[cache_key] = accurate_image_url
                
                # Update recipe with accurate image
                recipe['image_url'] = accurate_image_url
                recipe['image_source'] = 'accurate_recipe_specific'
            
            logger.debug(f"✅ Enhanced '{recipe_name}' with accurate image")
        
        logger.info(f"🖼️ Enhanced {len(recipes)} recipes with accurate images")
        return recipes
    ...
    async def search_recipes(
        ...
        # Enhance recipes with accurate images
        recipes = self._enhance_recipes_with_accurate_images(recipes)

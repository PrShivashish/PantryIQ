"""
PantryIQ: Intelligent Recipe Navigator with Algorithmic Insights
...
# --- START: CORS Middleware for Frontend Integration (FIXED) ---
# ALLOWING the live Render domain for the frontend to prevent CORS errors
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "https://pantryiq.onrender.com"], # **FIXED: Added Live Render URL**
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
# --- END: CORS Middleware for Frontend Integration ---
...
    logger.info("Starting PantryIQ API...")
    # The graph building is a resource-intensive task that must complete before the app serves
    await recipe_service.initialize()
    await graph_service.build_ingredient_graph()
    logger.info("PantryIQ API started successfully!")

@app.get("/")
async def root():
    """Health check endpoint"""
    return {"message": "PantryIQ API is running!", "version": "1.0.0"}
...
                # Expert accuracy fields
                match_percentage=recipe.get("match_percentage"),
                matched_ingredients=recipe.get("matched_ingredients"),
                total_matched=recipe.get("total_matched"),
                total_user_ingredients=recipe.get("total_user_ingredients")
            )
